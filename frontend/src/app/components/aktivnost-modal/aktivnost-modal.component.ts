...
export class AktivnostModalComponent implements OnChanges {
  ...
  dokumenti: Dokument[] = [];
  showDokumentiSidebar: boolean = false;
  ...
  loadDokumenti(): void {
    ...
    this.dokumentService.getByEntity('Aktivnost', this.aktivnost.id).subscribe({
      next: (data) => {
        this.dokumenti = data;
      },
      error: (error) => {
  ...
  uploadDokument(file: File): void {
    ...
    this.dokumentService.uploadForEntity('Aktivnost', this.aktivnost.id, file).subscribe({
      next: () => {
        this.loadDokumenti();
