-- Alter datum columns from datetime2 to date in ProjectImplementationItems and ProjectImplementationItemCheckLists
-- EF Migration: 20260401181245_AlterDatumFieldsToDateOnly

-- ProjectImplementationItems.ZavrsenoDatum
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[ProjectImplementationItems]') AND name = 'ZavrsenoDatum' AND system_type_id = TYPE_ID(N'datetime2'))
BEGIN
...
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[ProjectImplementationItems]') AND name = 'KlijentPotvrdioDatum' AND system_type_id = TYPE_ID(N'datetime2'))
BEGIN
...
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[ProjectImplementationItemCheckLists]') AND name = 'ZavrsenDatum' AND system_type_id = TYPE_ID(N'datetime2'))
BEGIN
...
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[ProjectImplementationItemCheckLists]') AND name = 'KlijentPotvrdioDatum' AND system_type_id = TYPE_ID(N'datetime2'))
BEGIN
